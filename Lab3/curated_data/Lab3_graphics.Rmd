...
Note that it is also possible to change the fonts of the plots. Remember we installed the cowplot package, this removes the gray background and increases the default font sizes in figures automatically. In the end there is less code to write! 
...
In reality the individual particpant IDs are published in figures for research journals, so let's hide them: 
```{r}
...
These are some basic manipulations using a scatter plot as an example, what if switch our type of plot?
```{r}
ggplot(data = scfa_qc, 
       mapping = aes(x = participant_id, 
                     y = butyrate_mean_mmol_kg, 
...
### Homework 3.5
Recreate the scatter plot from the above for each SCFA. Identify descriptive axis labels, make each plot a different color. Utilize copy and paste to reduce the amount of typing. Assign each figure to a new object. Website with some named colors: http://sape.inf.usi.ch/quick-reference/ggplot2/colour 
```{r}
plot_ace <- ggplot()
```
...
### With facets
Use the code above as a template to create a scatter plot. We need to pass some additional arguments to aesthetics without assignment them to an axis: supplement_consumed, semester, study_week.
