...
If not already imported into the global environment, import the sequence variant, taxa, and participant data files from Lab 7. We use the same information for calculating both alpha and beta diversity metrics.
...
sv_table <- read_delim("~/Documents/UMich_Bio201_F19/Lab7/seq_variants_wide.csv",
...
taxa <- read_delim("~/Documents/UMich_Bio201_F19/Lab7/seqvar_classifications.csv",
                            delim = ",", escape_double = FALSE, 
                            trim_ws = TRUE, na=c("NA"),
                            col_types = list()) %>%
...
all_data_indv <- read_delim("~/Documents/UMich_Bio201_F19/Lab7/all_data_indv.txt", 
                            delim = "\t", escape_double = FALSE, 
                            trim_ws = TRUE, 
...
As in lab 7 we are not using the Winter 2015 data, and we are only interested in semesters that have week 1 and week 3. We will make a subset of the phyloseq object then use that going forward. 
```{r}
physq_sub <- physq_obj %>%
...
The first step of the transformation process involves summarizing taxa at whatever rank (phylum through species) we plan to plot the data. We know from the previous code block we have 1349 taxa. #e will agglomerate (sum) the counts for ESVs which are members of the same phyla. Then we can view the number of taxa, and you should note how it is reduced compared to the starting data (1349 vs. 12).
```{r}
tmp1 <- physq_sub %>%
...
head(as.data.frame(otu_table(tmp2))) #otu table extract the counts, as.data.frame converts back to data frame class, head gives a preview of the data frame 
```

To improve computer performance remove any large objects you will not use downstream. 
```{r}
rm(tmp1)
rm(tmp2)
```

As you know ggplot requires a long-format data frame. In the code blocks above we are working with phyloseq data, which is matrix-derived and actually its own class of object, i.e., not a data frame! The phyloseq package contains a function, psmelt(), that takes a phyloseq object and converts it to a long-format data frame for us. We are going to pipe that function after the glom and transformation functions to create a data frame that is compatiable with ggplot(). Be carefule when using the psmelt() function, the new columns it creates do not follow the same column name style conventions we use in this course. 
```{r}
physq_long <- physq_sub %>%
  ...
  rename_all(tolower) %>% #make new columns match style conventions 
...
This step calculates the diveristy index (using Bray Curtis in the example below), and does the ranking process of the diveristy indices to generate the information necessary to create a two-dimensional plot. Notice the arguments in the ordinate function; to calculate Bray Curtis or Jaccard you specifiy in the distance agurment, with either of these metrics we will use method = NMDS. The place were you might want to make some modifications is the k (this should be somewhere between 1-3), the maximum number of iterations (more than 100 will take several minutes to calcaulte), and the inital number of interations to try (50 in the example below). These can be modified to reduce the stress, with the goal of a number less than 20 (the lower the better). 
```{r}
...
### Homework 8.3
Conduct the ordination analysis using Jaccard index.
```{r}
physq_j <- ordination()
```
...
For these plots we will use a phyloseq function plot_ordination() instead of ggplot(). 
```{r}
...
And this website for shape options: http://sape.inf.usi.ch/quick-reference/ggplot2/shape 
```{r}
ordplot2 <- ordplot1 + geom_point(size=3) + #increase point size
  scale_color_manual(name = "Semester",
                     values = c("coral3", "skyblue3", "goldenrod3"),
                     labels = c("Fall 2016",
                                "Winter 2016", 
...
If you observe a pattern (separation of points from different groups) in your ordination plot, we can determine if that is to a significant level with analysis of similarities (ANOSIM). Using the ordination object from the section above, we run the test with the anosim() function, which is part of the vegan package. 
```{r}
#start with same phyloseq object as above 
dat_bray <- phyloseq::distance(physq_sub, method = "bray") #calculate BC index 

sampledf <- physq_sub %>% 
  sample_data(.) %>% #extract sample data from phyloseq object 
  as(., "data.frame") #convert to data frame from adonis()
...
In our results above, the p-values of 0.001 indicate that the groupings of samples by semester or by study week is statistically significant. The R value of 0.025 and 0.008 are fairly close to 0, indicating that the grouping of samples based by semester or study week are weak. When this occurs (low p and low R), the variation of samples in the groups likely is influencing the results of the test. 
