...
```{r include=FALSE}
scfa_long <- read_delim("~/Box Sync/Schmidt_Lab/Bio201_teaching/Protocol_Rmd_docs/Lab5/raw_data/SCFA_wkly_long.txt", 
...
scfa_wide <- read_delim("~/Box Sync/Schmidt_Lab/Bio201_teaching/Protocol_Rmd_docs/Lab5/raw_data/SCFA_wkly_wide.txt", 
                        delim = "\t", col_names = TRUE, trim_ws = TRUE, na = c("NA")) %>%
  rename_all(tolower) %>%
  mutate(semester = factor(semester,
                           levels=c("Winter2015", "Fall2015", 
...
# Homework 5.2
Generate plots to determine if methane, acetate, propionate increased during week 3 of the study. Generate plot to determine if pH decreased during week 3 of the study. 
```{r}
# breath methane
```
